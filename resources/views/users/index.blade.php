        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @if($apiError || $errors->has('api'))
                <div class="flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-700 px-5 py-4 rounded-xl">
                    ...
                    <span class="text-sm"><strong>iHRIS API:</strong> {{ $apiError ?: $errors->first('api') }}</span>
                </div>
            @endif
            ...
            <div class="card" x-data="{
                userModalOpen: {{ ($errors->any() && (old('name') || old('role'))) ? 'true' : 'false' }},
                isEdit: {{ (old('_method') === 'PATCH' && (old('name') || old('role'))) ? 'true' : 'false' }},
                form: {
                    id: '{{ old('id') }}',
                    name: '{{ old('name') }}',
                    email: '{{ old('email') }}',
            ...
            @if($empTotal > 0)
            <div class="card" x-data="{
                empModalOpen: {{ ($errors->any() && (old('first_name') || old('last_name'))) ? 'true' : 'false' }},
                isEdit: {{ (old('_method') === 'PUT' && (old('first_name') || old('last_name'))) ? 'true' : 'false' }},
                empForm: {
                    id: '{{ old('id') }}',
                ...
                empActionUrl: '{{ (old('_method') === 'PUT') ? route('employees.update', old('id') ?? '0') : route('employees.store') }}',
                ...
                openEditEmp(emp) {
                    ...
                    this.empForm = {
                        id: emp.uuid || emp.id || emp.employee_id,
                ...
                >
                    ...
                    >
                        ...
                        <form method="POST" :action="empActionUrl" class="p-6 space-y-4">
                            ...
                            <input type="hidden" name="id" x-model="empForm.id">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                                <input type="email" name="email" x-model="empForm.email" class="lgu-input w-full" placeholder="email@example.com">
                            </div>
